...
function HealthBadge({ datasetId }: { datasetId: string }) {
  const health = useQuery({
    queryKey: ["dataset-health", datasetId],
    queryFn: () =>
      api<{
        freshness: string;
        quality_score?: number;
  ...
  if (health.isLoading || !health.data)
    return <span className="text-[11px] text-mute">a verificar…</span>;
  const failed = (health.data.checks || []).filter((check) => !check.ok).length;
  const tone = failed > 1 ? "danger" : failed === 1 ? "warn" : "ok";
  return (
    <Badge tone={tone}>
      {failed
        ? `${failed} ponto${failed === 1 ? "" : "s"} a rever`
        : "Pronto para IA"}
    </Badge>
  );
}
...
export default function DataPage() {
  ...
  const removeDataset = useMutation({
    mutationFn: (id: string) =>
      api(`/api/v1/datasets/${id}`, { method: "DELETE" }),
    onSuccess: () => {
    ...
    <div className="mx-auto max-w-5xl space-y-5">
        ...
        actions={
          <>
            ...
            <label className="inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-xl border border-line bg-surface px-3 text-sm font-medium text-ink shadow-sm transition hover:-translate-y-px hover:border-primary/25 hover:bg-surface-2">
              ...
              <input
                type="file"
                accept=".csv,.xlsx,.xls,.json,.ndjson"
                className="hidden"
                onChange={onUpload}
              />
            </label>
      ...
      {datasetList.length === 0 && !q.isLoading && !q.isError && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <EducationalCard
              title="1. Demo"
              body="50 mil linhas de vendas prontas para explorar."
            />
            <EducationalCard
              title="2. CSV / XLSX / JSON"
              body="Arraste um ficheiro e a TheDobra infere o esquema."
            />
            <EducationalCard
              title="3. Conectores"
              body="Hub com bases de dados, APIs, SaaS e streaming."
            />
          </div>
        </div>
      )}
              ...
              {datasetList.map((d) => (
                <tr key={d.id} className="border-t border-line hover:bg-bg">
                  ...
                  <Td>
                    <Badge
                      tone={
                        d.status === "ready" || d.status === "ready_ok"
                          ? "ok"
                  ...
                  <Td numeric>
                    {d.quality_score != null ? `${d.quality_score}/100` : "—"}
                  </Td>
                  <Td>
                    <HealthBadge datasetId={d.id} />
                  </Td>
                  <Td>
                    <Button
                      size="sm"
                      ...
                      onClick={() => {
                        if (
                          confirm(
                            `Excluir o conjunto «${d.name}»? Esta acção não se desfaz.`,
                          )
                        ) {
      ...
      {lakeList.length > 0 && (
        <details className="rounded-[1.125rem] border border-line bg-surface px-5 py-4 text-sm shadow-[var(--shadow-card)]">
          ...
          <p className="mb-3 mt-2 text-[12px] text-mute">
            Objetos ativos no data lake. Esta informação é útil para
