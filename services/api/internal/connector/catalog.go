...
var typeAliases = map[string]string{
	...
	"ml":                 "mercado_livre",
	"protheus":           "totvs_protheus",
	"totvs":              "totvs_protheus",
	"sapb1":              "sap_b1",
	"sap_business_one":   "sap_b1",
	"b1":                 "sap_b1",
	"senior_sistemas":    "senior",
	"ibge":               "ibge_censo",
	"censo":              "ibge_censo",
	"pnad":               "ibge_censo",
	"sidra":              "ibge_censo",
...
func Catalog() []Item {
	items := []Item{
		...
		{
			ID: "totvs_protheus", Label: "TOTVS Protheus", Group: GroupBrasil, GroupLabel: groupLabels[GroupBrasil],
			Description: "Protheus REST: clientes (SA1), pedidos (SC5), produtos (SB1), NF e fornecedores.", Implemented: true, Preview: false,
			Aliases: []string{"protheus", "totvs"},
			Fields: append(nameField(), []Field{
				{Key: "url", Label: "URL da API REST", Type: "url", Required: true, Placeholder: "https://servidor:porta/rest"},
				{Key: "token", Label: "Token / Bearer", Type: "password", Hint: "Token do Protheus REST (AdvPL)."},
				{Key: "user", Label: "Utilizador (Basic, opcional)", Type: "text"},
				{Key: "password", Label: "Senha (Basic, opcional)", Type: "password"},
				{Key: "database", Label: "Tenant / empresa", Type: "text", Hint: "Enviado no header tenantid."},
				{Key: "table", Label: "Recurso", Type: "select", Default: "customers", Options: []FieldOption{
					{Value: "customers", Label: "Clientes (SA1)"},
					{Value: "orders", Label: "Pedidos (SC5)"},
					{Value: "products", Label: "Produtos (SB1)"},
					{Value: "invoices", Label: "Notas fiscais (SF2)"},
					{Value: "vendors", Label: "Fornecedores (SA2)"},
				}},
				limitField(),
			}...),
		},
		{
			ID: "sap_b1", Label: "SAP Business One", Group: GroupBrasil, GroupLabel: groupLabels[GroupBrasil],
			Description: "Service Layer OData: login B1SESSION, paginação $top/$skip.", Implemented: true, Preview: false,
			Aliases: []string{"sapb1", "sap_business_one", "b1"},
			Fields: append(nameField(), []Field{
				{Key: "url", Label: "URL do Service Layer", Type: "url", Required: true, Placeholder: "https://servidor:50000"},
				{Key: "database", Label: "CompanyDB", Type: "text", Required: true, Placeholder: "SBODEMO"},
				{Key: "user", Label: "Utilizador", Type: "text", Required: true},
				{Key: "password", Label: "Senha", Type: "password", Required: true},
				{Key: "token", Label: "Session / token (opcional)", Type: "password", Hint: "Se já tiver B1SESSION, o login é omitido."},
				{Key: "table", Label: "Recurso", Type: "select", Default: "BusinessPartners", Options: []FieldOption{
					{Value: "BusinessPartners", Label: "Parceiros de negócio"},
					{Value: "Orders", Label: "Encomendas"},
					{Value: "Items", Label: "Artigos"},
					{Value: "Invoices", Label: "Faturas"},
					{Value: "PurchaseOrders", Label: "Compras"},
				}},
				limitField(),
			}...),
		},
		{
			ID: "senior", Label: "Senior", Group: GroupBrasil, GroupLabel: groupLabels[GroupBrasil],
			Description: "Senior HCM / Rubi via bridge REST (colaboradores, folha, férias).", Implemented: true, Preview: false,
			Aliases: []string{"senior_sistemas"},
			Fields: append(nameField(), []Field{
				{Key: "url", Label: "URL da API", Type: "url", Required: true, Placeholder: "https://plataforma.senior.com.br/t/senior.com.br/bridge/1.0"},
				{Key: "token", Label: "Token Bearer", Type: "password", Required: true},
				{Key: "user", Label: "Utilizador (opcional)", Type: "text"},
				{Key: "password", Label: "Senha (opcional)", Type: "password"},
				{Key: "table", Label: "Recurso", Type: "select", Default: "employees", Options: []FieldOption{
		...
		{
			ID: "ibge_censo", Label: "IBGE", Group: GroupEcon, GroupLabel: groupLabels[GroupEcon],
			Description: "SIDRA e localidades: PNAD Contínua (horas, renda, ocupação, informalidade, sexo, instrução, setor) e economia (PIB, valor adicionado, população e crescimento).", Implemented: true, Preview: false,
			Aliases: []string{"ibge", "censo", "pnad", "sidra"},
			Fields: append(nameField(), []Field{
				{Key: "table", Label: "Recurso", Type: "select", Default: "desocupacao", Options: []FieldOption{
					{Value: "municipios", Label: "Municípios (cadastro de localidades)"},
					{Value: "estados", Label: "Estados (cadastro de localidades)"},
					{Value: "horas", Label: "PNAD: horas trabalhadas por semana"},
					{Value: "rendimento", Label: "PNAD: rendimento médio real do trabalho"},
					{Value: "ocupacao", Label: "PNAD: taxa / nível de ocupação"},
					{Value: "desocupacao", Label: "PNAD: taxa de desocupação"},
					{Value: "informalidade", Label: "PNAD: taxa de informalidade"},
					{Value: "forca_trabalho", Label: "PNAD: taxa de participação"},
					{Value: "empregados", Label: "PNAD: ocupados por posição na ocupação"},
					{Value: "escolaridade", Label: "PNAD: ocupados por nível de instrução"},
					{Value: "sexo", Label: "PNAD: indicadores por sexo"},
					{Value: "setor", Label: "PNAD: ocupados por atividade económica"},
					{Value: "pib_brasil", Label: "PIB Brasil e per capita (contas nacionais)"},
					{Value: "pib_estados", Label: "PIB dos estados"},
					{Value: "pib_municipios", Label: "PIB dos municípios"},
					{Value: "pib_per_capita", Label: "PIB per capita dos estados"},
					{Value: "pib_per_capita_municipios", Label: "PIB per capita dos municípios"},
					{Value: "valor_adicionado", Label: "Valor adicionado (agro, indústria, serviços, admin.)"},
					{Value: "populacao", Label: "População estimada (estados)"},
					{Value: "populacao_municipios", Label: "População estimada (municípios)"},
					{Value: "crescimento_populacional", Label: "Crescimento populacional (estados)"},
				}},
				{Key: "url", Label: "URL SIDRA (opcional)", Type: "url", Hint: "Deixe vazio para a tabela oficial do recurso. Use para um agregado SIDRA específico."},
				{Key: "limit", Label: "Limite de linhas", Type: "number", Default: "50000", Hint: "PIB/população municipal pedem 30 mil+ linhas."},
			}...),
		},
		{
