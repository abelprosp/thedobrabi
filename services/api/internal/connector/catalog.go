...
func Catalog() []Item {
	items := []Item{
		...
		fileItem("xlsx", "Excel", ".xlsx,.xls,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel", "Upload .xlsx/.xls e ingestão da primeira folha.", []string{"Excel", "xls", "excel"}),
		{
			...
			Description: "Importa uma folha pública ou autenticada (Sheets API / OAuth).", Implemented: true, Preview: false,
			Aliases: []string{"gsheets", "planilha", "sheets", "googlesheets"},
			Fields: append(nameField(), []Field{
				{Key: "url", Label: "URL ou ID da planilha", Type: "text", Required: true, Placeholder: "https://docs.google.com/spreadsheets/d/…", Hint: "Cole o link completo ou só o ID. Se a planilha não for pública, indique uma chave de API ou um token OAuth."},
				{Key: "table", Label: "Folha (nome ou gid)", Type: "text", Placeholder: "Página1", Hint: "Vazio = primeira folha. O gid está no URL (#gid=123)."},
				{Key: "query", Label: "Intervalo A1 (opcional)", Type: "text", Placeholder: "A1:Z1000"},
				{Key: "api_key", Label: "Chave de API Google (opcional)", Type: "password", Hint: "Google Cloud → APIs e serviços → Sheets API. Não é necessária se a planilha for pública («qualquer pessoa com o link»)."},
				{Key: "token", Label: "OAuth access token (opcional)", Type: "password"},
				limitField(),
			}...),
		},
